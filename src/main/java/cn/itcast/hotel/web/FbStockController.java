...
@Slf4j
@RestController
@RequestMapping("fbStock")
@CrossOrigin(origins = "*")
public class FbStockController {
    @Resource
    ...
    public Result<DmCondition> getNumber(@RequestBody Map<String, String> params) throws Exception {
        ...
        CompletableFuture<Void> vpnCodeFuture = CompletableFuture.runAsync(() -> {
            if (dmCondition.getIsVpn() == 1) {
                dmCondition.setVpnCode(getVpnCodeSet(dmCondition, ip));
            } else {
    ...
    private Integer getVpnCodeSet(DmCondition dmCondition , String ip ){
